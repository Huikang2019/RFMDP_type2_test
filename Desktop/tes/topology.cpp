        ...
        for (int j = 0; j < number_of_states; j++) {
            for (int k = 0; k < number_of_states; k++) {
                for (int h = 0; h < number_of_computers; h++) {
                    switch (h % 3) {
                    case 0: epsilon_h = epsilon;
                        break;
                    case 1: epsilon_h = epsilon;
                        break;
                    case 2: epsilon_h = epsilon;
                        break;
                    }


                    double random_number = ((double)rand() / (RAND_MAX));
                    random_number = 2 * random_number - 1.0;
                    prob_real[2][i][j][k][h] = prob[2][i][j][k] - epsilon_h * random_number;
                    prob_real[1][i][j][k][h] = prob[1][i][j][k];
                    if (prob_real[2][i][j][k][h] < 0) {
                        prob_real[2][i][j][k][h] = 0.0;
                        prob_real[1][i][j][k][h] = prob[1][i][j][k] - epsilon_h * random_number + prob[2][i][j][k];
