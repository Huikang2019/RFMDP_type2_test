...
NumArray3D     weight, weight_half;
int type = 5;   // default: single-direct ring
...
int             state_half_constr[number_of_half][number_of_constraints];
...
int main(int argc, const char* argv[]) {
    ...
    //solve_overall_problem_scope_2();

    //solve_overall_problem_scope_2_robust();
    ...
    for (int iter = 0; iter < 70; iter++) {
        ...
        random_generate_constraints_local_search(weight, weight_half, 200, constr_num);
        if (constr_num >= number_of_constraints) break;
        value_new = mastered_problem_weight(constr_num, weight, weight_half);
        if ((value_new > 0) && (value_new < value_old * 0.99)) break;
